...
public class SRM646_CopyOfTheGridDivTwo {
	int max = 0;
	...
	private int moveUp(int px, int py, int[] x, int[] y, int k) {
		...
		if (block == false) {
			return moveTo(px + 1, py, x, y, k);
			//max =  moveTo(px + 1, py, x, y, k);
		}
		...
		if (blockup == false) {
			return moveTo(px, py + 1, x, y, k);
			//max = Math.max( max, moveTo(px, py + 1, x, y, k));
		}
	...
	private int moveRight(int px, int py, int[] x, int[] y, int k ) {
	...
	private int moveTo(int px, int py, int[] x, int[] y, int k ) {
		// Add blocking point...
		int[] cx = Arrays.copyOf(x, x.length + 1);
		int[] cy = Arrays.copyOf(y, y.length + 1);
